...
##############################################
######### Special for DPMM critic ############
##############################################
class DPMM_Critic(base_component.Component):
    ...
    def forward(
        ...
        latent_obs: TensorType, # Input both latent variable & env obs

        action: TensorType,
        detach_encoder: bool = False,
    ) -> Tuple[TensorType, TensorType]:

        assert latent_obs.size(0) == action.size(0) # batch num

        # obs_action = torch.cat([latent_obs, action], dim=-1) # concat latent obs with action
        obs_action = torch.cat([mtobs.env_obs, latent_obs, action], dim=-1)
...
###
###
class TransformerQFunction(base_component.Component):
    def __init__(
        self,
        input_dim: int,
        #action_dim: int,
    ...
    def build_model(
        ...
        mods: List[nn.Module] = agent_utils._get_list_of_layers_with_dropout(
            input_dim=input_dim,
            hidden_dim=hidden_dim,
            output_dim=1,
            num_layers=num_layers,
    ...
    def forward(self, encoding_action: TensorType) -> TensorType:
        return self.model(encoding_action)

class TransformerCritic(base_component.Component):
    def __init__(
        self,
        env_obs_shape: List[int],
        action_shape: List[int],
        ...
        self.env_flatten_dim = env_obs_shape[0]
        self.action_flatten_dim = action_shape[0]
        self.dropout = dropout

        #model_input_dim = transformer_encoder_cfg.transformer.compression_dim
        if use_tra_preprocessing:
            model_input_dim = tra_preprocessing_dim + self.env_flatten_dim
        elif use_cls_prediction_head or use_task_id:
            model_input_dim = transformer_encoder_cfg.prediction_head_cls.latent_dim 
        elif transformer_encoder_cfg.transformer.compression_dim != 0:
            model_input_dim = transformer_encoder_cfg.transformer.compression_dim
        else:
    ...
    def forward(
        ...
        #obs_action = torch.cat([encoding, action], dim=-1)
