...
class VAE_Encoder(Encoder):
    def __init__(
        ...
        self.input_dim = env_obs_shape[0] + 768
        self.env_shape = env_obs_shape[0]
        
        # encoder
        mods = []
        last_dim = self.input_dim
        for layer in hidden_dims_encoder:
            mods += [nn.Linear(last_dim, layer), nn.ReLU()]
            last_dim = layer
        mods += [nn.Linear(last_dim, latent_dim*2)]
        self.encoder = nn.Sequential(*mods)
        # self.encoder = self.build_mlp(input_dim=self.input_dim, 
        ...
        mods = []
        last_dim = latent_dim
        for layer in hidden_dims_decoder:
            mods += [nn.Linear(last_dim, layer), nn.ReLU()]
            last_dim = layer
        mods += [nn.Linear(last_dim, 768+env_obs_shape[0])]
        self.decoder = nn.Sequential(*mods)
        # self.decoder = self.build_mlp(
        #     # input_dim=latent_dim + env_obs_shape[0] + 4,
        #     input_dim=latent_dim,
        #     hidden_dim=hidden_dim,
        #     output_dim=768+env_obs_shape[0], # hard code dim, we reconstruct embedding
        #     num_layers=num_layers+1,
        #     output_activation=False
        # )
        print(f"Encoder {self.encoder} decoder {self.decoder}")
    ...
    def forward(self, mtobs: MTObs, batch:ReplayBufferSample, 
        ...
        mu_and_log_var = self.encoder(env_obs)
        mu, log_var = mu_and_log_var.chunk(2, dim=-1)
        ...
        if should_reconst and batch is not None:
            ...
            reconst['context'], reconst['next_obs'] = torch.split(reconstruction, [768, self.env_shape], dim=-1)
            #reconst['context'] = torch.tanh(reconst['context'])
            # reconst['reward'] = self.decoder_reward(decoder_inputs)
            
            # reconst['next_obs'] = self.decoder_state(z)
