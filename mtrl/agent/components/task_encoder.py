...
class TaskEncoder(base_component.Component):
    def __init__(
        ...
        if pretrained_embedding_cfg.should_use:
            ...
            #if "-v2-0" in ordered_task_list[0]:
            #    ordered_task_list = [ele[:-2] for ele in ordered_task_list]
    ...
    def forward(self, env_index: TensorType) -> TensorType:
        ...
        embedding = self.pretrained_embedding(env_index)
        if len(embedding.shape) == 1:
            return embedding[None]
        return embedding
        # Andi: old: return self.pretrained_embedding(env_index)
