...
class TransformerActor(BaseActor):
    ...
    def make_model(
        ...
        if self.use_tra_preprocessing:
            model_input_dim = self.tra_preprocessing_dim + self.env_flatten_dim
        elif self.use_cls_prediction_head or self.use_task_id:
    ...
    def forward(
        self,
        state_encoding: TensorType,
    ) -> Tuple[TensorType, TensorType, TensorType, TensorType]:
        
        mu_and_log_std = self.model(state_encoding)
