...
class Agent(AbstractAgent):
    ...
    # andi
    def compute_q_target_and_policy_density_from_batch(self, batch):
        if self.should_use_task_encoder:
            task_encoding = self.get_task_encoding(
                env_index=batch.task_obs.squeeze(1),
                disable_grad=True,
        ...
        with torch.no_grad():
            _, policy_action, log_pi, _ = self.actor(mtobs=mtobs)
            target_Q1, target_Q2 = self.critic_target(mtobs=mtobs, action=policy_action)
            q_target = torch.min(target_Q1, target_Q2) - self.get_alpha(env_index=batch.task_obs).detach() * log_pi
            ...
            teacher_mu, _, _, teacher_log_std = self.actor(mtobs=mtobs)

        return q_target, teacher_mu, teacher_log_std
    
    def compute_q_target_and_policy_density(self, env_obs, next_env_obs, task_obs):
        task_info = TaskInfo(
                encoding=None, compute_grad=False, env_index=task_obs
            )
        mtobs = MTObs(env_obs=next_env_obs, task_obs=None, task_info=task_info)
        with torch.no_grad():
            _, policy_action, log_pi, _ = self.actor(mtobs=mtobs)
            target_Q1, target_Q2 = self.critic_target(mtobs=mtobs, action=policy_action)
            q_target = torch.min(target_Q1, target_Q2) - self.get_alpha(env_index=task_obs).detach() * log_pi
    ...
    def update_with_reward_shapening(
        self,
        replay_buffer: ReplayBuffer,
        logger: Logger,
        step: int,
        ...
        with torch.no_grad():
            target_V = self._get_target_V(batch=batch, task_info=task_info)
            target_Q = batch.reward + expert_weight * batch.q_target + (batch.not_done * self.discount * target_V)
