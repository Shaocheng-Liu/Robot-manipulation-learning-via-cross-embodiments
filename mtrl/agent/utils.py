...
def _get_list_of_layers_with_dropout(
    input_dim: int, hidden_dim: int, output_dim: int, num_layers: int, dropout,
) -> List[nn.Module]:
    """Utility function to get a list of layers. This assumes all the hidden
    layers are using the same dimensionality.
    ...
    if num_layers == 0:
        ...
        mods = [nn.Linear(input_dim, hidden_dim), nn.ReLU(), nn.Dropout(p=dropout)]
        for _ in range(num_layers - 1):
            mods += [nn.Linear(hidden_dim, hidden_dim), nn.ReLU(), nn.Dropout(p=dropout)]
        mods.append(nn.Linear(hidden_dim, output_dim))
    return mods
